        ...
        <div className="w-full grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
          {[
            ...
            },
            {
              name: "한 동 환",
              intro: `전체회원
              - 팔로잉 수 상위 10% 회원을 인기유저로, 차단 건수 10회 이상 회원을 블랙유저로 자동 분류하는 기능 구현
              - 이름이 같은 회원 구분을 위한 해시태그로 데이터 로드
              - 회원리스트 페이지네이션 처리 (일방향 페이지네이션)
              - 검색 조건에 맞는 회원 리스트 노출 기능
              - 회원 별 차단 및 팔로우 기능 구현
              - 인라인 프로필 프리뷰를 통해 자주 찾는 정보를 제공하고, 다이렉트 링크로 쉽게 이동할 수 있게 구현
              마이페이지
              - 프로필 사진 등록시 React-easy-crop 라이브러리를 이용한 사진 편집 후 파일파싱 기능 구현
               (blob형태로 반환 된 데이터 확장자 및 이름 파싱 로직 구현)
              - PapaParse 라이브러리를 이용해 방대한 주소 데이터 스트리밍 방식으로 파싱 후 조회 기능 구현
              - 알람 수신 동의 데이터 변경 시 Trigger 활용 하여, 변경 날짜 로그 기록
              공지사항
              - 공지사항 타입별 조회기능 구현 (공지,FAQ,프로모션)
              - CKEditor라이브러리를 이용한 본문 작성 기능 구현
              - 공지사항 페이지네이션 처리 (숫자형 페이지네이션)
              - 공지사항 수정 및 삭제 기능 구현
              - 공지사항 조회 수 증가 기능 구현`,
            },
          ].map((member, index) => (

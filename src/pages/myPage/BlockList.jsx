...
export default function BlockList({ memberNo }) {
  ...
  const fetchBlockList = async (page, size) => {
    try {
      const [data, profilesData] = await Promise.all([
        MemberListAPI.getBlockList(memberNo, page, size),
  ...
  useEffect(() => {
    fetchBlockList(currentPage, size);
  ...
  return (
    <div className="w-full mx-auto mb-10 mt-14 px-4 sm:px-6 lg:px-8">
      <div className="
        ...
        gap-4"
      >
        {loading ? renderSkeletonCards(size) : renderMemberCards(blockList)}
      </div>
